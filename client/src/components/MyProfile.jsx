...
export default function MyProfile({ onBack }) {
  ...
  const activeGs = activeGroup ? groupStats[activeGroup] : null;
      ...
      <div className="mp-content">
        <div className="mp-grid">
          ...
          <div className="mp-col-right">
            ...
            {activeGs ? (
              <div className="mp-card">
                <div className="mp-card-header">
                  <h3 className="mp-card-title">{activeMembership?.group_name}</h3>
                  <div className="mp-power-badge" style={{
                    background: `${getPowerTier(activeGs.powerScore).color}15`,
                    color: getPowerTier(activeGs.powerScore).color
                  }}>
                    {activeGs.powerScore} 战力
                  </div>
                </div>
                ...
                <div className="mp-roles">
                  {Object.entries(ROLE_INFO).map(([key, info]) => {
                    const rs = activeGs.roleStats?.[key] || { games: 0, wins: 0, winRate: 0 };
                    return (
                ...
                {/* Top Characters */}
                {activeGs.topCharacters?.length > 0 && (
                  <div className="mp-chars">
                    <h4 className="mp-card-subtitle">擅长角色</h4>
                    {activeGs.topCharacters.slice(0, 5).map((c, i) => (
            ...
            {allGames.length > 0 && (
              <div className="mp-card">
                <h3 className="mp-card-title">历史战绩 <span className="mp-count-badge">{allGames.length}</span></h3>
                <div className="mp-history">
                  {allGames.slice(0, 30).map((game, i) => {
